...
import { Plus, Shield, ChevronRight, LogIn } from 'lucide-react';
          ...
          <div className="bg-white w-full max-w-sm rounded-[26px] sm:rounded-[32px] shadow-2xl p-6 sm:p-8 border border-slate-100 animate-fade-in-up max-h-[calc(100dvh-2rem)] sm:max-h-none overflow-y-auto">
            ...
            <form onSubmit={handleAdminSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full text-slate-400 text-xs font-bold hover:text-slate-600 py-2"
              >
                Voltar
              </button>
            </form>
      ...
      <div className="w-full max-w-[400px] bg-white rounded-[28px] sm:rounded-[40px] shadow-2xl overflow-hidden flex flex-col animate-fade-in-up max-h-[calc(100dvh-2rem)] sm:max-h-none">
        <div className="bg-white px-6 sm:px-8 pt-8 sm:pt-12 pb-8 sm:pb-12 text-center">
          ...
          <p className="text-slate-400 text-sm font-medium leading-relaxed">
            Desenvolva sua fluidez conversando
            <br />
            com nossa inteligencia artificial.
          </p>
        </div>

        <div className="px-6 sm:px-8 pb-8 sm:pb-12 space-y-4 overflow-y-auto">
          <button
            onClick={onCreateNew}
            className="w-full bg-indigo-600 text-white p-4 sm:p-5 rounded-3xl shadow-xl shadow-indigo-100 flex items-center justify-between group transition-all hover:bg-indigo-700"
          >
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-2xl bg-white/10 flex items-center justify-center">
                <Plus className="w-5 h-5" />
              </div>
              <span className="font-bold text-base sm:text-lg">Criar Perfil</span>
            </div>
              ...
              {users.length === 0 ? (
                ...
                users.map(user => (
                  <button
                    ...
                    className="w-full bg-slate-50/60 p-4 rounded-2xl flex items-center gap-4 hover:bg-slate-100 transition-colors group"
                  >
                    <div className={`w-10 h-10 rounded-2xl ${user.avatarColor} flex items-center justify-center text-white font-black text-xs shadow-sm`}>
